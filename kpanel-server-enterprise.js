  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      db.exec(createTables, (err) => {
        ...
        db.get("SELECT id FROM users WHERE email = ?", [CONFIG.adminEmail], (err, row) => {
          if (err) {
            reject(err);
            return;
          }
...
async function initializeServer() {
  try {
    ...
    log.info(`💾 Memory: ${serverInfo.memory.free}GB / ${serverInfo.memory.total}GB`);
    ...
    if (fs.existsSync(clientBuildPath)) {
      ...
      log.warn("Client build not found. Please run: cd client && npm run build");
    ...
    const server = app.listen(CONFIG.port, "0.0.0.0", () => {
      ...
      console.log(`   📱 Control Panel: http://${serverInfo.publicIP}:${CONFIG.port}`);
      console.log(`   🏠 Local: http://localhost:${CONFIG.port}`);
      console.log(`   💻 LAN: http://${serverInfo.localIP}:${CONFIG.port}`);
      console.log("");
      
      console.log("🔗 API Endpoints:");
      console.log(`   🏥 Health: http://${serverInfo.publicIP}:${CONFIG.port}/api/health`);
      console.log(`   🔐 Login: http://${serverInfo.publicIP}:${CONFIG.port}/api/auth/login`);
      console.log(`   📊 System: http://${serverInfo.publicIP}:${CONFIG.port}/api/system/info`);
      console.log("");
